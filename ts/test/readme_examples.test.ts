...
//     (\`new DemonSlayerSDK(...)\` / \`DemonSlayerSDK.test(...)\` become
//     \`DemonSlayerSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"character":{"test01":{"id":"test01"}},"combat_style":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new DemonSlayerSDK(...)\` and any
// \`DemonSlayerSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+DemonSlayerSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/DemonSlayerSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
