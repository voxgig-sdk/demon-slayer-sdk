...
class TestCombatStyleEntity:

    def test_should_create_instance(self):
        testsdk = DemonSlayerSDK.test(None, None)
        ent = testsdk.CombatStyle(None)
    ...
    def test_should_stream(self):
        ...
        base = DemonSlayerSDK.test(seed, None)
        seen = list(base.CombatStyle(None).stream("list", None, None))
        assert len(seen) == 3

        # Inbound: streaming active -> yields each item from the feature.
        from demonslayer_sdk.config import shared_config
        cfg = shared_config()
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            sdk = DemonSlayerSDK.test(
                seed, {"feature": {"streaming": {"active": True}}})
            got = []
            for item in sdk.CombatStyle(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        setup = _combat_style_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["list", "load"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "combat_style." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set DEMON_SLAYER_TEST_COMBAT_STYLE_ENTID JSON to run live")
        client = setup["client"]

        # Bootstrap entity data from existing test data.
        combat_style_ref01_data_raw = vs.items(helpers.to_map(
            vs.getpath(setup["data"], "existing.combat_style")))
        combat_style_ref01_data = None
        if len(combat_style_ref01_data_raw) > 0:
        ...
        combat_style_ref01_data_dt0_load_result = helpers.to_map(runner.entity_data(combat_style_ref01_data_dt0_loaded))
...
def _combat_style_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/combat_style/CombatStyleTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("DEMON_SLAYER_TEST_COMBAT_STYLE_ENTID"))
    if idmap_resolved is None:
        idmap_resolved = helpers.to_map(idmap)

    if env.get("DEMON_SLAYER_TEST_LIVE") == "TRUE":
        merged_opts = vs.merge([
            {
            },
            extra or {},
        ])
        client = DemonSlayerSDK(helpers.to_map(merged_opts))

    _live = env.get("DEMON_SLAYER_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("DEMON_SLAYER_TEST_EXPLAIN") == "TRUE",
        "live": _live,
