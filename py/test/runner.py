...
class DemonSlayerTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            DemonSlayerTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = DemonSlayerTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = DemonSlayerTestRunner.getenv("DEMON_SLAYER_TEST_LIVE")
        override = DemonSlayerTestRunner.getenv("DEMON_SLAYER_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = DemonSlayerTestRunner.getenv(key)
        ...
        explain = DemonSlayerTestRunner.getenv("DEMON_SLAYER_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["DEMON_SLAYER_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if DemonSlayerTestRunner._test_control is not None:
            return DemonSlayerTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                DemonSlayerTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            DemonSlayerTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = DemonSlayerTestRunner.load_test_control()
...
def load_env_local():
    DemonSlayerTestRunner.load_env_local()


def env_override(m):
    return DemonSlayerTestRunner.env_override(m)


def entity_data(v):
    return DemonSlayerTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return DemonSlayerTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return DemonSlayerTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return DemonSlayerTestRunner.load_test_control()


def live_client_options():
    return DemonSlayerTestRunner.live_client_options()


def live_delay_ms():
    return DemonSlayerTestRunner.live_delay_ms()
