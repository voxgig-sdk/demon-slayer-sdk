...
from demonslayer_sdk.demonslayer_types import (
    CombatStyle,
    CombatStyleLoadMatch,
    CombatStyleListMatch,
)
...
class CombatStyleEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return CombatStyleEntity(self._client, opts)
    ...
    def match_get(self) -> CombatStyle:
    ...
    def load(self, reqmatch=None, ctrl=None) -> CombatStyle:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.CombatStyle().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[CombatStyle]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.CombatStyle().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
        ctx = utility.make_context({
            "opname": "list",
