...
from demonslayer_sdk.core.control import DemonSlayerControl
...
from demonslayer_sdk.core.helpers import get_ctx_prop, to_map
...
class DemonSlayerContext:
    def __init__(self, ctxmap=None, basectx=None):
        ...
        if isinstance(sp, DemonSlayerSpec):
            self.spec = sp
        elif basectx is not None:
        ...
        if isinstance(r, DemonSlayerResult):
            self.result = r
        elif basectx is not None:
        ...
        if isinstance(rp, DemonSlayerResponse):
            self.response = rp
        elif basectx is not None:
    ...
    def make_error(self, code, msg):
        return DemonSlayerError(code, msg, self)
