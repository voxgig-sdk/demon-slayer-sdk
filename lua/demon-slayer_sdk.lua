...
local DemonSlayerSDK = {}
DemonSlayerSDK.__index = DemonSlayerSDK
...
DemonSlayerSDK._make_feature = _make_feature


function DemonSlayerSDK.new(options)
  local self = setmetatable({}, DemonSlayerSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function DemonSlayerSDK:options_map()
...
function DemonSlayerSDK:get_utility()
  return Utility.copy(self._utility)
end


function DemonSlayerSDK:get_root_ctx()
  return self._rootctx
end


function DemonSlayerSDK:prepare(fetchargs)
...
function DemonSlayerSDK:direct(fetchargs)
...
-- by passing it.
function DemonSlayerSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function DemonSlayerSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "DemonSlayerSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function DemonSlayerSDK:CombatStyle(data)
  ...
  if data == nil then
    if self._combat_style == nil then
      self._combat_style = EntityMod.new(self, nil)
    end
...
function DemonSlayerSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = DemonSlayerSDK.new(sdkopts)
  sdk.mode = "test"
