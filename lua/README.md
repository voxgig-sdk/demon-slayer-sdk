...
The Lua SDK for the DemonSlayer API — an entity-oriented client using Lua conventions.
...
| --- | --- | --- |
| `abilities` | `table` | List of abilities and techniques the character possesses |
...
| `quotes` | `table` | Memorable quotes from the character |
...
Create an instance: `local combat_style = client:CombatStyle(nil)`
...
| `forms` | `table` | List of forms or techniques within this combat style |
...
| `users` | `table` | Characters who use this combat style |

#### Example: Load

```lua
local combat_style, err = client:CombatStyle():load({ id = "combat_style_id" })
```

#### Example: List

```lua
local combat_styles, err = client:CombatStyle():list()
```
...
```
lua/
├── demon-slayer_sdk.lua    -- Main SDK module
...
The main module (`demon-slayer_sdk`) exports the SDK constructor
