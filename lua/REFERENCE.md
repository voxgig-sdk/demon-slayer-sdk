# DemonSlayer Lua SDK Reference

Complete API reference for the DemonSlayer Lua SDK.
...
```lua
local combat_style = client:CombatStyle(nil)
```
...
```lua
local results, err = client:CombatStyle(nil):list(nil, nil)
```
...
```lua
local result, err = client:CombatStyle(nil):load({ id = "combat_style_id" }, nil)
```
