-- Character entity test

local json = require("dkjson")
local vs = require("utility.struct.struct")
local sdk = require("demon-slayer_sdk")
local helpers = require("core.helpers")
local runner = require("test.runner")

local _test_dir = debug.getinfo(1, "S").source:match("^@(.+/)")  or "./"

describe("CharacterEntity", function()
  it("should create instance", function()
    local testsdk = sdk.test(nil, nil)
    local ent = testsdk:Character(nil)
    assert.is_not_nil(ent)
  end)

  it("should run basic flow", function()
    local setup = character_basic_setup(nil)
    -- Per-op sdk-test-control.json skip.
    local _live = setup.live or false
    for _, _op in ipairs({"list", "load"}) do
      local _should_skip, _reason = runner.is_control_skipped("entityOp", "character." .. _op, _live and "live" or "unit")
      if _should_skip then
        pending(_reason or "skipped via sdk-test-control.json")
        return
      end
    end
    -- The basic flow consumes synthetic IDs from the fixture. In live mode
    -- without an *_ENTID env override, those IDs hit the live API and 4xx.
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set DEMONSLAYER_TEST_CHARACTER_ENTID JSON to run live")
      return
    end
    local client = setup.client

    -- Bootstrap entity data from existing test data.
    local character_ref01_data_raw = vs.items(helpers.to_map(
      vs.getpath(setup.data, "existing.character")))
    local character_ref01_data = nil
    if #character_ref01_data_raw > 0 then
      character_ref01_data = helpers.to_map(character_ref01_data_raw[1][2])
    end

    -- LIST
    local character_ref01_ent = client:Character(nil)
    local character_ref01_match = {}

    local character_ref01_list_result, err = character_ref01_ent:list(character_ref01_match, nil)
    assert.is_nil(err)
    assert.is_table(character_ref01_list_result)

    -- LOAD
    local character_ref01_match_dt0 = {
      id = character_ref01_data["id"],
    }
    local character_ref01_data_dt0_loaded, err = character_ref01_ent:load(character_ref01_match_dt0, nil)
    assert.is_nil(err)
    local character_ref01_data_dt0_load_result = helpers.to_map(character_ref01_data_dt0_loaded)
    assert.is_not_nil(character_ref01_data_dt0_load_result)
    assert.are.equal(character_ref01_data_dt0_load_result["id"], character_ref01_data["id"])

  end)
end)

function character_basic_setup(extra)
  runner.load_env_local()

  local entity_data_file = _test_dir .. "../../.sdk/test/entity/character/CharacterTestData.json"
  local f = io.open(entity_data_file, "r")
  if f == nil then
    error("failed to read character test data: " .. entity_data_file)
  end
  local entity_data_source = f:read("*a")
  f:close()

  local entity_data = json.decode(entity_data_source)

  local options = {}
  options["entity"] = entity_data["existing"]

  local client = sdk.test(options, extra)

  -- Generate idmap via transform.
  local idmap = vs.transform(
    { "character01", "character02", "character03" },
    {
      ["`$PACK`"] = { "", {
        ["`$KEY`"] = "`$COPY`",
        ["`$VAL`"] = { "`$FORMAT`", "upper", "`$COPY`" },
      }},
    }
  )

  -- Detect ENTID env override before envOverride consumes it. When live
  -- mode is on without a real override, the basic test runs against synthetic
  -- IDs from the fixture and 4xx's. Surface this so the test can skip.
  local entid_env_raw = os.getenv("DEMONSLAYER_TEST_CHARACTER_ENTID")
  local idmap_overridden = entid_env_raw ~= nil and entid_env_raw:match("^%s*{") ~= nil

  local env = runner.env_override({
    ["DEMONSLAYER_TEST_CHARACTER_ENTID"] = idmap,
    ["DEMONSLAYER_TEST_LIVE"] = "FALSE",
    ["DEMONSLAYER_TEST_EXPLAIN"] = "FALSE",
    ["DEMONSLAYER_APIKEY"] = "NONE",
  })

  local idmap_resolved = helpers.to_map(
    env["DEMONSLAYER_TEST_CHARACTER_ENTID"])
  if idmap_resolved == nil then
    idmap_resolved = helpers.to_map(idmap)
  end

  if env["DEMONSLAYER_TEST_LIVE"] == "TRUE" then
    local merged_opts = vs.merge({
      {
        apikey = env["DEMONSLAYER_APIKEY"],
      },
      extra or {},
    })
    client = sdk.new(helpers.to_map(merged_opts))
  end

  local live = env["DEMONSLAYER_TEST_LIVE"] == "TRUE"
  return {
    client = client,
    data = entity_data,
    idmap = idmap_resolved,
    env = env,
    explain = env["DEMONSLAYER_TEST_EXPLAIN"] == "TRUE",
    live = live,
    synthetic_only = live and not idmap_overridden,
    now = os.time() * 1000,
  }
end
