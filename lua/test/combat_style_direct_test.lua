...
describe("CombatStyleDirect", function()
  it("should direct-list-combat_style", function()
    local setup = combat_style_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-combat_style", setup.live and "live" or "unit")
  ...
  it("should direct-load-combat_style", function()
    local setup = combat_style_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-combat_style", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function combat_style_direct_setup(mockres)
