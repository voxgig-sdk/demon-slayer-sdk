...
local CombatStyleEntity = {}
CombatStyleEntity.__index = CombatStyleEntity


function CombatStyleEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, CombatStyleEntity)
  self._name = "combat_style"
  self._client = client
...
function CombatStyleEntity:get_name()
  return self._name
end


function CombatStyleEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return CombatStyleEntity.new(self._client, opts)
end
...
function CombatStyleEntity:mark_deleted()
  self._deleted = true
end


function CombatStyleEntity:deleted()
  return true == self._deleted
end


function CombatStyleEntity:data_set(args)
  if args ~= nil then
...
function CombatStyleEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function CombatStyleEntity:match_set(args)
  if args ~= nil then
...
function CombatStyleEntity:match_get()
...
function CombatStyleEntity:stream(action, args, callopts)
...
function CombatStyleEntity:_run_op(ctx, post_done)
  local utility = self._utility
