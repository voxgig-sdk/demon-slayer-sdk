...
func TestCombatStyleEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.CombatStyle(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.CombatStyle(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		combatStyleRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.combat_style")))
		...
		combatStyleRef01DataDt0LoadResult := core.ToMapAny(entityData(combatStyleRef01DataDt0Loaded))
...
func combat_styleBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("DEMON_SLAYER_TEST_COMBAT_STYLE_ENTID")
	...
	idmapResolved := core.ToMapAny(env["DEMON_SLAYER_TEST_COMBAT_STYLE_ENTID"])
