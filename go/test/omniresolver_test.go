...
import (
	...
	omni "github.com/voxgig-sdk/demon-slayer-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.DemonSlayerError); is {
				out := map[string]any{
					"name":    "DemonSlayerError",
...
func omniCtx(arg any, client *sdk.DemonSlayerSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.DemonSlayerSDK); is {
			client = live
