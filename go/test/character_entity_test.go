...
func TestCharacterEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set DEMONSLAYER_TEST_CHARACTER_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func characterBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("DEMONSLAYER_TEST_CHARACTER_ENTID")
	...
	idmapResolved := core.ToMapAny(env["DEMONSLAYER_TEST_CHARACTER_ENTID"])
