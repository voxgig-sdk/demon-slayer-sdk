...
class DemonSlayerSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $utility = new DemonSlayerUtility();
        ...
        $feature_opts = DemonSlayerHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = DemonSlayerHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!DemonSlayerFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, DemonSlayerFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return DemonSlayerUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = DemonSlayerHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = DemonSlayerHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = DemonSlayerHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new DemonSlayerError($op . "_allow",
                "DemonSlayerSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = DemonSlayerHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new DemonSlayerError("graphql_error",
                "DemonSlayerSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    private $_combat_style = null;

    // Canonical facade: $client->CombatStyle()->list() / ->load(["id" => ...]).
    // PHP method names are case-insensitive, so lowercase $client->combat_style()
    // resolves here too.
    public function CombatStyle($data = null)
    {
        require_once __DIR__ . '/entity/combat_style_entity.php';
        if ($data === null) {
            if ($this->_combat_style === null) {
                $this->_combat_style = new CombatStyleEntity($this, null);
            }
            return $this->_combat_style;
        }
        return new CombatStyleEntity($this, $data);
    }
    ...
    public static function test(?array $testopts = null, ?array $sdkopts = null): self
    {
        ...
        $sdk = new DemonSlayerSDK($sdkopts);
        $sdk->mode = "test";
        return $sdk;
    }
}
