...
// DemonSlayer SDK CombatStyle entity
...
class CombatStyleEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new CombatStyleEntity($this->_client, $opts);
    }

    /**
     * @param CombatStyle|array $args CombatStyle data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = DemonSlayerHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return CombatStyle|array The current CombatStyle data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of CombatStyle fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = DemonSlayerHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of CombatStyle fields).
     */
     ...
     *
     * @param CombatStyleLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed CombatStyleLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return CombatStyle|array The loaded CombatStyle as an assoc-array at the
     *   SDK boundary; throws DemonSlayerError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = DemonSlayerHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
     ...
     *
     * @param CombatStyleListMatch|array|null $reqmatch Match filter (any subset
     *   of CombatStyle fields) as an assoc-array; CombatStyleListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return CombatStyle[]|array A list of CombatStyle items as assoc-arrays at
     *   the SDK boundary; throws DemonSlayerError on failure (item-5 convention).
     */
