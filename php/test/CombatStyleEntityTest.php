...
class CombatStyleEntityTest extends TestCase
{
    ...
    public function test_stream(): void
    {
        ...
        $base = DemonSlayerSDK::test($seed, null);
        $seen = iterator_to_array($base->CombatStyle(null)->stream("list", null, null), false);
        $this->assertCount(3, $seen);

        // Inbound: streaming active -> yields each item from the feature.
        $cfg = DemonSlayerConfig::shared_config();
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            $sdk = DemonSlayerSDK::test($seed, ["feature" => ["streaming" => ["active" => true]]]);
            $got = [];
            foreach ($sdk->CombatStyle(null)->stream("list", null, null) as $item) {
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set DEMON_SLAYER_TEST_COMBAT_STYLE_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $combat_style_ref01_data_dt0_load_result = Helpers::to_map(is_object($combat_style_ref01_data_dt0_loaded) && method_exists($combat_style_ref01_data_dt0_loaded, 'data_get') ? $combat_style_ref01_data_dt0_loaded->data_get() : $combat_style_ref01_data_dt0_loaded);
...
function combat_style_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("DEMON_SLAYER_TEST_COMBAT_STYLE_ENTID");
