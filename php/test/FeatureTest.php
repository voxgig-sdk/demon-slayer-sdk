...
class FtCtrl extends DemonSlayerControl
{
...
class FtClient
{
    ...
    private DemonSlayerUtility $utility;

    public function __construct(DemonSlayerUtility $utility, array $options)
    {
    ...
    public function get_utility(): DemonSlayerUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (DemonSlayerContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public DemonSlayerContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (DemonSlayerContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = DemonSlayerConfig::make_feature($name);
    ...
    private static function buildUrl(DemonSlayerSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof DemonSlayerError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new DemonSlayerResponse($fetched);
        }
    ...
    private function fail(DemonSlayerContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (DemonSlayerContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
