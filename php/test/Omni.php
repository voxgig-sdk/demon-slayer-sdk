...
//    only - a typed DemonSlayerContext there would make every ctx
...
final class DemonSlayerOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new DemonSlayerContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new DemonSlayerSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new DemonSlayerResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new DemonSlayerError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new DemonSlayerResponse($clean['response']);
    ...
    public static function err_from_map($m): ?DemonSlayerError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new DemonSlayerError(is_string($code) ? $code : '', $msg);
    }
