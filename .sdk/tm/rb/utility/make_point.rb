...
module DemonSlayerUtilities
  MakePoint = ->(ctx) {
    if ctx.out["point"]
      ...
      return nil, preset if preset.is_a?(DemonSlayerError)
      ctx.point = preset
    ...
    if op.points.length == 1
      ...
      matched = false
      op.points.each do |p|
        select_def = DemonSlayerHelpers.to_map(VoxgigStruct.getprop(p, "select"))
        found = true
      ...
      if reqselector
        req_action = VoxgigStruct.getprop(reqselector, "$action")
        if req_action && point
          point_select = DemonSlayerHelpers.to_map(VoxgigStruct.getprop(point, "select"))
