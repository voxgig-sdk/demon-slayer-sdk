...
module DemonSlayerOmni
  ...
  class ObjView < ::Hash
    ...
    def view_wrap(name)
      ...
      return val if DemonSlayerOmni.plainval?(val) || seen.key?(val.object_id)
    ...
    def view_attrname(key)
      ...
      snake = DemonSlayerOmni.underscore(name)
    ...
    def keys
      out = view_names.map { |n| DemonSlayerOmni.camelize(n) }
    ...
    def []=(key, val)
      ...
      t = @view_target
      name = view_attrname(key) || DemonSlayerOmni.underscore(key.to_s)
      setter = "#{name}="
  ...
  def enrich(ctxmap, ctx)
    ...
    ctx.spec = DemonSlayerSpec.new(spec_map) if spec_map.is_a?(::Hash)
    ...
    if res_map.is_a?(::Hash)
      ...
      if err_map.is_a?(::Hash) && err_map['message'].is_a?(::String) &&
        ...
        ctx.result.err = DemonSlayerError.new('', err_map['message'])
      end
    end
  ...
  def sdkhooks(sdk)
    ...
    errify = ->(err) { DemonSlayerOmni.errify(err) }
