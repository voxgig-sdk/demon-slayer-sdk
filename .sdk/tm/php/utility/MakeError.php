...
class DemonSlayerMakeError
{
    public static function call(?DemonSlayerContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new DemonSlayerContext([], null);
        }
        $op = $ctx->op ?? new DemonSlayerOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new DemonSlayerResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof DemonSlayerError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new DemonSlayerError('', $msg, $ctx);
        ...
        if ($err instanceof DemonSlayerError) {
