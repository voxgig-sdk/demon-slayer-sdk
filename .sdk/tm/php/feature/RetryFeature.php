...
class DemonSlayerRetryFeature extends DemonSlayerBaseFeature
{
    ...
    public function init(DemonSlayerContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (DemonSlayerContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(DemonSlayerContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(DemonSlayerContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
