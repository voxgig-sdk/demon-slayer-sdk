...
boru-driven command-line client **and** interactive REPL for the DemonSlayer
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/demon-slayer-cli)
make build

# 2. See usage (words, entities, env vars)
./demon-slayer-cli --help

# 3. Provide credentials once, via the environment
export DEMON_SLAYER_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./demon-slayer-cli list character
./demon-slayer-cli load 1 character            # {id:1} shorthand
./demon-slayer-cli load '{id:1}' character       # explicit match map
./demon-slayer-cli list combat_style

# 5. Override the API base URL for a single call
DEMON_SLAYER_BASE=https://api.example.com ./demon-slayer-cli list character

# 6. No arguments -> interactive REPL
./demon-slayer-cli
demon-slayer> list character
demon-slayer> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/demon-slayer-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/demon-slayer-cli list character
   ```
...
```sh
./demon-slayer-cli load 1 character          # scalar shorthand for {id:1}
./demon-slayer-cli load '{id:1}' character     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `demon-slayer>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/demon-slayer-cli`. |
...
(`./demon-slayer-cli <expr>`) and interactively (the REPL), and expressions compose the
