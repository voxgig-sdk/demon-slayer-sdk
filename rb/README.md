# DemonSlayer Ruby SDK



The Ruby SDK for the DemonSlayer API — an entity-oriented client using idiomatic Ruby conventions.
...
```ruby
client = DemonSlayerSDK.test(testopts, sdkopts)
```
...
| `CombatStyle` | `(data) -> CombatStyleEntity` | Create a CombatStyle entity instance. |
...
raise a `DemonSlayerError` (a `StandardError` subclass), so wrap
...
| --- | --- | --- |
| `abilities` | `Array` | List of abilities and techniques the character possesses |
...
| `quotes` | `Array` | Memorable quotes from the character |
...
Create an instance: `combat_style = client.CombatStyle`
...
| `forms` | `Array` | List of forms or techniques within this combat style |
...
| `users` | `Array` | Characters who use this combat style |
...
```ruby
# list returns an Array of CombatStyle records (raises on error).
combat_styles = client.CombatStyle.list
```
...
```
rb/
├── DemonSlayer_sdk.rb       -- Main SDK module
...
The main module (`DemonSlayer_sdk`) exports the SDK class
