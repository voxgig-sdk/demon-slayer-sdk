...
require_relative 'DemonSlayer_types'
...
class DemonSlayerSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = DemonSlayerHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = DemonSlayerHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, DemonSlayerFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    DemonSlayerUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = DemonSlayerHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = DemonSlayerHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = DemonSlayerHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = DemonSlayerHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = DemonSlayerError.new(
        "graphql_error", "DemonSlayerSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = DemonSlayerSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
