...
class CombatStyleEntity
  ...
  def make
    opts = @_entopts.dup
    CombatStyleEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = DemonSlayerHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = DemonSlayerHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  #   as an empty match, so client.CombatStyle.load works with no args).
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = DemonSlayerHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
  ...
  #
  # @param reqmatch [CombatStyleListMatch, Hash, nil] match filter (any subset of
  #   CombatStyle fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
