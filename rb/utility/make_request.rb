...
module DemonSlayerUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = DemonSlayerResponse.new({})
    ...
    if fetch_err
      ...
      response = DemonSlayerResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = DemonSlayerResponse.new(fetched)
    else
