...
[MCP](https://modelcontextprotocol.io) server exposing the DemonSlayer SDK as
two agent tools — `demon-slayer_list` and `demon-slayer_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/demon-slayer-mcp)
make build

# 2. Provide credentials via the environment
export DEMON_SLAYER_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user demon-slayer \
  -- /absolute/path/to/demon-slayer-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./demon-slayer-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// demon-slayer_list: first page of records
{ "entity": "character" }
{ "entity": "character", "query": { } }

// demon-slayer_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/demon-slayer-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user demon-slayer \
     -- "$PWD"/dist/*/demon-slayer-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `demon-slayer_list` and `demon-slayer_load` tools now appear
   in new sessions. Ask the agent to *"list character using demon-slayer"*
   and it calls `demon-slayer_list` with `{"entity":"character"}`.
...
```sh
export DEMON_SLAYER_APIKEY=sk_live_xxx            # API key
export DEMON_SLAYER_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `demon-slayer_list` tool
...
### Call the `demon-slayer_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `demon-slayer_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `demon-slayer_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `DEMON_SLAYER_APIKEY` | API key sent with every request. |
| `DEMON_SLAYER_BASE` | Optional override of the API base URL. |
...
```sh
./demon-slayer-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"demon-slayer_load","arguments":{"entity":"character","query":{"id":1}}}}'
```
